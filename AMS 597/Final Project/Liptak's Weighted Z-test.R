...
weighted.z.test <- function(x, y, alternative=c("two.sided", "greater", "equal"), mu=0, var.equal=F, conf.level=0.95){
  ...
  if (alternative == "greater"){
    p1 <- t.test(x[which(!is.na(x)&!is.na(y))], y[which(!is.na(x)&!is.na(y))], alternative = "greater", paired = T, var.equal=var.equal, conf.level = conf.level)$p.value
    p2 <- t.test(x[which(!is.na(x)&is.na(y))], y[which(is.na(x)&!is.na(y))], alternative = "greater", paired = F, var.equal=var.equal, conf.level = conf.level)$p.value
    Z1 <- qnorm(1-p1)
    Z2 <- qnorm(1-p2)
    ...
    p1 <- t.test(x[which(!is.na(x)&!is.na(y))], y[which(!is.na(x)&!is.na(y))], alternative = "less", paired = T, var.equal=var.equal, conf.level = conf.level)$p.value
    p2 <- t.test(x[which(!is.na(x)&is.na(y))], y[which(is.na(x)&!is.na(y))], alternative = "less", paired = F, var.equal=var.equal, conf.level = conf.level)$p.value
    Z1 <- qnorm(1-p1)
    Z2 <- qnorm(1-p2)
    p <- 1 - pnorm((w1*Z1 + w2*Z2)/sqrt(w1^2 + w2^2))
  ...
  result <- list(p.value=p, weight1=w1, weight2=w2, Z.score1=Z1, Z.score2=Z2)
  return(result)
}
